...
const ModelMetrics: React.FC<Props> = ({ metrics }) => {
  ...
  // Determine mode: Regression (RMSE passed in f1_score) vs Classification
  // In predictor.py, we set accuracy=0 for regression, and pass RMSE in f1_score.
  const isRegression = accuracy === 0 && f1_score > 0;
  const isUnsupervised = accuracy === 0 && f1_score === 0;

  // Safe destructuring for Classification mode
  const [[tp, fp], [fn, tn]] = (confusion_matrix && confusion_matrix.length === 2)
    ? confusion_matrix
    : [[0, 0], [0, 0]];
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg">
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-[10px] text-slate-500 font-mono mt-1">
            {isRegression
              ? `Temporal Regression Evaluation (Test Set N=${dataset_size.toLocaleString()})`
              : isUnsupervised
                ? "Unsupervised Risk Engine"
                : `Classifier Evaluation (Test Set N=${dataset_size.toLocaleString()})`
            }
          </p>
        </div>
        <div className="text-right">
          {isRegression ? (
            <>
              <div className="text-2xl font-bold text-emerald-400">{f1_score.toFixed(2)}</div>
              <div className="text-[10px] text-slate-500 uppercase tracking-wider">RMSE (Error)</div>
            </>
          ) : (
            <>
              <div className="text-2xl font-bold text-white">{isUnsupervised ? "N/A" : `${(accuracy * 100).toFixed(1)}%`}</div>
      ...
      {isUnsupervised ? (
        <div className="flex flex-col items-center justify-center h-48 bg-slate-950/50 rounded-lg border border-slate-800/50 border-dashed">
          <AlertTriangle className="text-amber-500 mb-2" size={24} />
          <p className="text-slate-400 text-xs font-medium">Metrics Not Available</p>
          <p className="text-slate-500 text-[10px] max-w-xs text-center mt-1">
            Not enough data for supervised evaluation. Using heuristic fallback.
          </p>
        </div>
      ) : isRegression ? (
        <div className="grid grid-cols-1 gap-4">
          <div className="bg-slate-950 p-4 rounded-lg border border-slate-800/60 flex items-center justify-between">
            <div>
              <span className="text-[10px] text-slate-500 uppercase tracking-wider block mb-1">Root Mean Squared Error</span>
              <span className="text-xl font-bold text-emerald-400">{f1_score.toFixed(2)}</span>
              <p className="text-[9px] text-slate-600 mt-1">Average deviation from actual crime volume.</p>
            </div>
            <div className="h-12 w-1 bg-emerald-500/20 rounded-full"></div>
          </div>
          <div className="bg-blue-900/10 border border-blue-900/30 p-3 rounded text-xs text-blue-200">
            <p><strong>Note:</strong> This is a regression model predicting <em>volume</em> of crimes. Traditional classification metrics (Precision/Recall) do not apply.</p>
          </div>
        </div>
      ) : (
